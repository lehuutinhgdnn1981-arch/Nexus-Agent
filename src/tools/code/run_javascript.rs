//! run_javascript tool — execute JavaScript via Node.
...
impl Tool for RunJavaScriptTool {
    fn name(&self) -> &'static str { "run_javascript" }

    fn description(&self) -> &'static str {
        "Execute JavaScript (Node.js) code in an isolated workspace. Requires `node` on PATH."
    }
    ...
    async fn execute(&self, ctx: &ToolContext, input: serde_json::Value) -> Result<ToolResult> {
        ...
        let node = which::which("node")
            .map_err(|_| NexusError::NotFound("node not found on PATH".into()))?;

        let script_path = ctx.workspace.resolve_with_parents(&format!(".nexus_scripts/{}.js", crate::utils::ids::short_id()))?;
