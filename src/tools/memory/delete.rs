...
impl Tool for MemoryDeleteTool {
    ...
    fn description(&self) -> &'static str { "Delete a long-term memory by ID. DANGEROUS — irreversible." }
    ...
    async fn execute(&self, ctx: &ToolContext, input: serde_json::Value) -> Result<ToolResult> {
        ...
        ctx.memory.delete(id).await?;
        Ok(ToolResult::ok("", self.name(), format!("deleted memory {id}")))
    }
}
